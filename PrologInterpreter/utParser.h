...
TEST(ParserTest, createTerm_Struct)
{
...
TEST(ParserTest, createTerm_underscoredVar)
{
...
TEST(ParserTest, listofTermsTwoNumber)
{
...
TEST(ParserTest, createTerm_nestedStruct2) {
	Scanner scanner("s(s(s(s(1))))");
	Parser parser(scanner);
	ASSERT_EQ("s(s(s(s(1))))", parser.createTerm()->symbol());
}

TEST(ParserTest, createTerm_nestedStruct3) {
...
TEST(ParserTest, createTerm_DotStruct) {
	Scanner scanner("...(11,12)");
	Parser parser(scanner);
	ASSERT_EQ("...(11, 12)", parser.createTerm()->symbol());
}

TEST(ParserTest, createTerm_emptyList) {
	Scanner scanner("   [   ]");
	Parser parser(scanner);
	ASSERT_EQ("[]", parser.createTerm()->symbol());
}

TEST(ParserTest, createTerm_listWithTwoTerms) {
	Scanner scanner("   [1, 2]");
	Parser parser(scanner);
	ASSERT_EQ("[1, 2]", parser.createTerm()->symbol());
}

TEST(ParserTest, createTerm_nestedList) {
	Scanner scanner("   [  [1], [] ]");
	Parser parser(scanner);
	ASSERT_EQ("[[1], []]", parser.createTerm()->symbol());
}

TEST(ParserTest, createTerm_ListOfListAndStruct) {
	Scanner scanner("   [  [1], [], s(s(1)) ]   ");
	Parser parser(scanner);
	ASSERT_EQ("[[1], [], s(s(1))]", parser.createTerm()->symbol());
}

TEST(ParserTest, createTerm_illeageTerm) {
	Scanner scanner("[1,2)");
	Parser parser(scanner);
	try {
		parser.createTerm();
...
TEST(ParserTest, createTerm_ListAsStruct) {
	Scanner scanner(".(1,[])");
	Parser parser(scanner);
	Term* term = parser.createTerm();
...
TEST(ParserTest, createTerm_ListAsStruct2) {
	Scanner scanner(".(2,.(1,[]))");
	Parser parser(scanner);
	Term* term = parser.createTerm();
...
TEST(ParserTest, OneMatchingFalse) {
	Scanner scanner("1=2.");
	Parser parser(scanner);
...
TEST(ParserTest, TwoTermsMatching) {
...
TEST(ParserTest, ThreeTermsMatching) {
...
TEST(ParserTest, TwoVariableMatching2) {
...
TEST(ParserTest, VarAStructOfVar) {
...
TEST(ParserTest, TwoVariableMatching4) {
...
TEST(ParserTest, ConjTwoMatchingFailure) {
...
TEST(ParserTest, DisjTwoMatchingSuccess) {
	Scanner scanner("X=1; X=2.");
	Parser parser(scanner);
	parser.matchings();
	vector<Term *> terms = parser.getTerms();
